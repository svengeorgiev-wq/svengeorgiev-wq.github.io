...
- Fixed navigation labels: `Heute`, `21 Tage`, `22 Tools`, `Audios`, `Belege`. The core model is the manuscript's `Regulations-Ampel` with the four questions `Tempo`, `Atem`, `Spannung`, `Impuls`.
...
- The audio library contains seven book-faithful Eleven-v3 episodes with spoken chapter/tool cross-references, Ramona voice continuity, no autoplay, and 720 ms verified leading silence in every final MP3. Audio files are cached on demand when first played.
- Current shell query is `v=2`; service-worker cache is `overthinking-dating-v1.1.0`. Shell changes must advance both.
