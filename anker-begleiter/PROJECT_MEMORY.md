...
- Local source: `C:\Users\sveng\OneDrive\Dokumente\Bonus-App\anker-begleiter`.
...
- Current source manuscript: `6.9 ADHS-Wechseljahre-Interior-DRUCKFERTIG.pdf`; the app implements the printed seven-module promise and manuscript-specific night-plan and physician-intro wording.
...
- Previous published baseline: resource query `v=6`, service-worker cache `anker-begleiter-v1.0.5`.
- Current public refined glass release: resource query `v=11`, service-worker cache `anker-begleiter-v2.1.2`. The system uses layered translucent plum glass, edge highlights, backdrop saturation, gold refractions, and opaque fallbacks. The start and Today screens include the exact install CTA `Auf den Homescreen legen` plus expandable iPhone/iPad, Android, and desktop instructions; native install prompts are used when available and the instructions open as fallback. The Today home adds the supplied book cover as a compact, persistent recognition tile. The user authorized deployment on 2026-09-06.
...
- The prepared icon keeps the anchor mark but uses the final cover palette (plum, cream, gold). `assets/book-cover-v1.webp` is the optimized in-app cover; `assets/book-cover-v1.png` preserves the supplied source image.
